...
const OutgoingCallWidget: React.FC<OutgoingCallWidgetProps> = ({ name, imgSrc, onEndCall }) => {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex flex-col justify-between items-center py-12 px-4 bg-slate-950/95 text-white backdrop-blur-2xl animate-in fade-in duration-200 overflow-hidden">
            ...
            {/* Top Status Header */}
            <div className="flex flex-col items-center text-center space-y-2 mt-4 z-10">
                ...
                >
                    {isGroup ? (
                        <div className="flex items-center -space-x-4">
                            {imgSrc.slice(0, 3).map((src, idx) => (
                                <img
                                    ...
                                    alt=""
                                    className="h-24 w-24 sm:h-32 sm:w-32 rounded-3xl object-cover ring-4 ring-slate-900 shadow-2xl border border-cyan-500/40"
                                    onError={(e) => {
                        ...
                        <img
                            ...
                            alt={name}
                            className="h-28 w-28 sm:h-36 sm:w-36 rounded-3xl object-cover ring-4 ring-cyan-500/50 shadow-2xl shadow-cyan-500/20"
                            onError={(e) => {
                                (e.target as HTMLImageElement).src = DEFAULT_AVATAR;
                            }}
                        />
            ...
            <div className="flex flex-col items-center space-y-3 mb-6 z-10">
                <button
                    type="button"
                    onClick={onEndCall}
                    className="group relative flex items-center justify-center h-16 w-16 sm:h-18 sm:w-18 rounded-full bg-rose-600 hover:bg-rose-500 text-white shadow-xl shadow-rose-600/30 hover:scale-105 active:scale-95 transition-all cursor-pointer"
                    aria-label="End call"
                >
                    <PhoneOff className="h-7 w-7 sm:h-8 sm:w-8 group-hover:rotate-12 transition-transform" />
                </button>
