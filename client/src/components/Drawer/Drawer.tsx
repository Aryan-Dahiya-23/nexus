...
import { Conversation, Participant, UserConversationRef } from "../../types";
...
const Drawer: React.FC<DrawerProps> = ({ name, avatarSrc }) => {
    ...
    // Try reading cached conversation, fallback to user's conversation list
    const cachedConversation = queryClient.getQueryData<Conversation>(['chats', id]);
    const fallbackConv = user?.conversations?.find(
        (c: UserConversationRef) => c?.conversation?._id === id
    )?.conversation;
    const conversation = cachedConversation || fallbackConv;
    ...
    // Exclude current user from participant list if already present to avoid duplicate "You" row
    const rawParticipants: Participant[] = conversation?.participants || [];
    const participantsList: Participant[] = rawParticipants.filter(
        (p) => p._id !== user?._id
    );
            ...
            {isOpen && (
                ...
                >
                    <motion.div
                        ...
                        className="w-full max-w-sm sm:max-w-md h-[100dvh] bg-card text-card-foreground border-l border-border p-5 sm:p-6 shadow-2xl flex flex-col justify-between overflow-y-auto relative z-[101]"
                    ...
                    >
                        ...
                        <div className="space-y-6 pt-2">
                            ...
                            <div className="space-y-2.5">
                                ...
                                <div className="space-y-1.5 max-h-64 overflow-y-auto custom-scrollbar rounded-2xl border border-border/70 p-1.5 bg-background/50">
                                    {/* Current User Row */}
                                    {user && (
    ...
    return (
        <>
            ...
            {typeof document !== "undefined" ? createPortal(drawerContent, document.body) : drawerContent}
        </>
    );
};
